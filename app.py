...
with tabs[0]:
    ...
    # Calculate Audits & Revenue metrics dynamically from active dataset
        ...
        if not records_data:
            ...
            # Maintain a clean count of total selected items in the basket across all search/filter states
            selected_count = len(st.session_state.selected_uuids)
            ...
            if user["role"] in ["Super Admin", "Manager", "Auditor"]:
                # 1. Grab the rows (using all_raw_items to ensure hidden/filtered-out selections are still fully exported)
                if selected_count > 0:
                    full_df = pd.DataFrame(all_raw_items)
                    export_df = full_df[full_df['id'].isin(st.session_state.selected_uuids)].copy()
                    # Keep visual sort consistency even when exporting cross-filtered selections
                    if 'sort_col_key' in locals() and sort_col_key in export_df.columns:
                        export_df = export_df.sort_values(by=sort_col_key, ascending=asc)
                else:
                    export_df = df.copy()
                # Dynamic mapping to support both raw database columns and pre-cleaned
                # display columns
                column_mapping = {
                ...
                print_df["Signature"] = ""
                export_label = f"Export {selected_count} Selected Rows" if selected_count > 0 else f"Export All {len(print_df)} Filtered Rows"
            ...
            new_active_id = selected_ids[0] if len(selected_ids) == 1 else None
            if st.session_state.get("active_select_id") != new_active_id:
                st.session_state.active_select_id = new_active_id
                # Automatically wipe any unsaved input states so they don't bleed into the new record
                ...
                if record_item:
                    ...
                    with col_mod:
                        ...
                        if lock_owner:
                                    ...
                                    if fresh_owner:
                                        ...
                                        try:
                                            ...
                                            if "Success" in result:
                                                ...
                                                st.session_state[f"edit_committed_{db_id}"] = False
                                                # Wipe cached dirty inputs to guarantee clean future edits
                                                for k in list(st.session_state.keys()):
                                                    if k.startswith("edit_"):
                                                        st.session_state.pop(k, None)
                            ...
                            with col_edit_act2:
                                if st.button("Cancel ❌", key="edit_cancel_btn",
                                use_container_width=True):
                                    if cloud_sync.SYNC_ACTIVE:
                                        cloud_sync.release_edit_lock(str(db_id))
                                    st.session_state.active_select_id = None
                                    st.session_state[f"edit_committed_{db_id}"] = False
                                    # Wipe dirty text input states so they revert to original database values
                                    for k in list(st.session_state.keys()):
                                        if k.startswith("edit_") and k != "edit_cancel_btn":
                                            st.session_state.pop(k, None)
                                    st.session_state.table_id = st.session_state.get("table_id", 0) + 1
                                    st.rerun()
                        else:
                    ...
                    if is_importing:
                        try:
                            ...
                            for i, (idx, row) in enumerate(rows_to_import.iterrows()):
                                row_time = import_base_time + timedelta(microseconds=i)
