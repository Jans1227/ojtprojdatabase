...
TARGET_DRIVE = os.getenv("GDRIVE_LETTER", "G").upper().replace(":", "").replace("\\", "").replace("/", "").strip()
CLOUD_SYNC_DIR = os.getenv("CLOUD_SYNC_DIR", fr"{TARGET_DRIVE}:\My Drive\Tanza_DB_Sync")
...
def claim_main_server_turf(server_name: str) -> bool:
    ...
    if os.path.exists(turf_file):
        try:
            # Check if WE already own the active turf before assuming it's locked by another PC
            with open(turf_file, "r") as f:
                if f.readline().strip() == MY_NODE_ID:
                    os.utime(turf_file, None)
                    AM_I_LEADER = True
                    return True
        ...
        for file_path in changesets:
            ...
            try:
                ...
                # Last-Write-Wins (LWW) check
                    ...
                    if table == "user":
                        # Solve username seeding collisions (e.g., independently seeded 'admin' accounts)
                        conflict_user = db_session.query(Model).filter(Model.username == clean_data["username"]).first()
                        if conflict_user:
                            db_session.delete(conflict_user)
                            db_session.flush()
    ...
    try:
        ...
        with database.get_db() as db_session:
            ...
            for f in outbox_files:
                ...
                if base.startswith("change_"):
                    # Extract the exact {table_key}_{rec_id} reliably even if the table name has underscores
                    core_parts = base[7:].rsplit('_', 1)
                    if len(core_parts) == 2:
                        pending_exports.add(core_parts[0])
